...
struct RecordingAidsOptions {
    let showClicks: Bool
    let showKeys: Bool
    let enableAnnotation: Bool
}
...
final class RecordingAidsOverlay {
    ...
    func show(
        ...
        guard options.showClicks || options.showKeys || options.enableAnnotation else { return }
        ...
        model.resetForSession(
            ...
            enableAnnotation: options.enableAnnotation,
            existingStrokes: annotationState.snapshot()
        )
        applyInteractionMode()
        panel?.orderFrontRegardless()

        if options.showClicks, clickMonitor == nil {
        ...
        if options.showKeys, keyMonitor == nil {
