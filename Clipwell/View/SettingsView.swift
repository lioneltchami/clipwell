...
/// The settings window for Clipwell
    ...
    var body: some View {
        Form {
            Section("Recording") {
                ...
                KeyboardShortcuts.Recorder("Pause / Resume", name: .togglePauseRecording)
            }
            ...
            Section("Camera") {
                KeyboardShortcuts.Recorder("Cycle Camera Layout", name: .cycleCameraLayout)
                KeyboardShortcuts.Recorder("Camera Small", name: .cameraLayoutSmall)
                KeyboardShortcuts.Recorder("Camera Large", name: .cameraLayoutLarge)
                KeyboardShortcuts.Recorder("Camera Bar", name: .cameraLayoutBar)
                KeyboardShortcuts.Recorder("Camera Hidden", name: .cameraLayoutHidden)
            }

            Section("Annotations") {
                KeyboardShortcuts.Recorder("Toggle Draw Mode", name: .toggleAnnotationDrawMode)
                KeyboardShortcuts.Recorder("Clear Annotations", name: .clearAnnotations)
            }

            Section {
                Text("Shortcuts work globally, even when Clipwell is not focused. macOS may reserve some combinations; verify each shortcut after setting it.")
                    .font(.caption)
    ...
    var body: some View {
        Form {
            ...
            Section("Display Elements") {
                ...
                Toggle("Show Clipwell", isOn: $settings.showClipwell)
            }
    ...
    var body: some View {
        Form {
            ...
            Section("Recording") {
                ...
                Toggle("Show trim and GIF after save", isOn: $settings.showPostRecordingSheet)
                Toggle("Highlight mouse clicks", isOn: $settings.showClickHighlighter)
                Toggle("Show keystroke overlay", isOn: $settings.showKeystrokeOverlay)
                Toggle("Enable annotation layer", isOn: $settings.enableAnnotationLayer)
                Text("While recording, use Toggle Draw Mode to draw on the capture region. Annotations are burned into the saved video.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
    ...
    var body: some View {
        Section("About") {
            ...
            LabeledContent("Website") {
                Link("lioneltchami.github.io/clipwell", destination: URL(string: "https://lioneltchami.github.io/clipwell/")!)
            }

            LabeledContent("Source Code") {
                Link("github.com/lioneltchami/clipwell", destination: URL(string: "https://github.com/lioneltchami/clipwell")!)
            }
        }
    }
}
