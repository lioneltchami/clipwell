    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            MenuBarExpandableSection(title: "Content Filter") {
                ...
                MenuBarToggle(name: "Show Clipwell", isOn: $settings.showClipwell)
            }
...
struct PresenterOverlaySettingsSection: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Picker("Camera mode", selection: $settings.cameraCaptureMode) {
            ...
            if settings.cameraCaptureMode != .off {
                Text(settings.cameraCaptureMode.detail)
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .padding(.horizontal, 12)
                    .padding(.bottom, 6)
            ...
            if settings.cameraCaptureMode == .builtInOverlay {
                Text("During recording, use the floating bar or shortcuts to switch Small / Large / Bar / Hidden.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .padding(.horizontal, 12)
                    .padding(.bottom, 8)
