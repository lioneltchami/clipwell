...
final class CaptureEngine: NSObject {
    ...
    private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "Clipwell", category: "CaptureEngine")

    // Queues for sample buffer handling
    private let videoSampleQueue = DispatchQueue(label: "com.clipwell.videoSampleQueue", qos: .userInteractive)
    private let audioSampleQueue = DispatchQueue(label: "com.clipwell.audioSampleQueue", qos: .userInteractive)
    private let microphoneSampleQueue = DispatchQueue(label: "com.clipwell.microphoneSampleQueue", qos: .userInteractive)
    ...
    func setPaused(_ paused: Bool) {
        ...
        logger.info("\(paused ? "Capture paused" : "Capture resumed")")
    }
    ...
    private func createStreamConfiguration(from settings: SettingsStore, contentSize: CGSize, sourceRect: CGRect? = nil) -> SCStreamConfiguration {
        ...
        if settings.cameraCaptureMode == .systemPresenterOverlay {
...
extension CaptureEngine: SCStreamOutput {
    ...
    nonisolated func stream(_ stream: SCStream, didOutputSampleBuffer sampleBuffer: CMSampleBuffer, of type: SCStreamOutputType) {
        ...
        switch type {
        case .screen:
            sampleBufferDelegate?.captureEngine(didOutputVideoSampleBuffer: sampleBuffer)
        case .audio:
            sampleBufferDelegate?.captureEngine(didOutputAudioSampleBuffer: sampleBuffer)
        case .microphone:
            sampleBufferDelegate?.captureEngine(didOutputMicrophoneSampleBuffer: sampleBuffer)
        @unknown default:
            break
        }
    }
