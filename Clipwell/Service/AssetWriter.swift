...
nonisolated final class AssetWriter: CaptureEngineSampleBufferDelegate, @unchecked Sendable {
    ...
    private let logger = Logger(
        subsystem: Bundle.main.bundleIdentifier ?? "Clipwell", category: "AssetWriter")
    ...
    private let videoDrainQueue = DispatchQueue(
        label: "com.clipwell.assetWriter.videoDrain", qos: .userInitiated)
    ...
    /// Optional annotation strokes composited onto each video frame.
    ...
    func appendVideoSample(_ sampleBuffer: CMSampleBuffer) {
        ...
        lock.withLockUnchecked {
            ...
            if let annotationState {
                let strokes = annotationState.snapshot()
                if !strokes.isEmpty {
                    AnnotationCompositor.composite(strokes: strokes, onto: pixelBuffer)
                }
            }
    ...
    /// Compensates capture timestamps for a pause interval so resumed samples stay
    /// contiguous on the output timeline.
    func compensateForPause(duration: TimeInterval) {
        guard duration > 0 else { return }
        lock.withLockUnchecked {
            guard hasStartedSession else { return }
            let offset = CMTime(seconds: duration, preferredTimescale: 600)
            sessionAnchor = CMTimeAdd(sessionAnchor, offset)
            logger.info("Compensated pause duration: \(duration, format: .fixed(precision: 3))s")
        }
    }
...
extension AssetWriter {
    ...
    func captureEngine(didOutputVideoSampleBuffer sampleBuffer: CMSampleBuffer) {
        appendVideoSample(sampleBuffer)
    }

    func captureEngine(didOutputAudioSampleBuffer sampleBuffer: CMSampleBuffer) {
        appendAudioSample(sampleBuffer)
    }

    func captureEngine(didOutputMicrophoneSampleBuffer sampleBuffer: CMSampleBuffer) {
