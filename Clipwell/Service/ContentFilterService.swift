...
final class ContentFilterService {
    ...
    private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "Clipwell", category: "ContentFilterService")
    ...
    func applySettings(to filter: SCContentFilter, settings: SettingsStore) async throws -> SCContentFilter {
        ...
        // If both wallpaper and dock are shown and Clipwell is shown, no need to rebuild the filter
        if settings.showWallpaper && settings.showDock && settings.showClipwell {
        ...
        for window in availableWindows {
            ...
            // Exclude Clipwell's own windows if showClipwell is false
            if !settings.showClipwell && bundleID == Bundle.main.bundleIdentifier {
                excludedWindows.append(window)
                logger.debug("Excluding Clipwell window: \(windowTitle)")
                continue
            }
