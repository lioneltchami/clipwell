...
final class RecorderViewModel {
    ...
    private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "Clipwell", category: "RecorderViewModel")
    ...
    private func getContentSize(from filter: SCContentFilter) async -> CGSize {
        ...
        if let screen = NSScreen.main {
            fallbackSize = screen.frame.size
        } else {
        ...
        return Self.outputSize(
            ...
            useNativeResolution: settings.captureNativeResolution,
            fallbackSize: fallbackSize
        )
    }

    /// Returns the capture dimensions in logical points or native pixels.
    /// The fallback size is expressed in logical points and follows the same resolution mode.
    static func outputSize(
        ...
        let rect = sourceRect ?? contentRect
        let size = sourceRect != nil || (rect.width > 0 && rect.height > 0)
            ? rect.size
            : fallbackSize
        let scale = useNativeResolution ? pointPixelScale : 1
