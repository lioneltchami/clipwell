...
Clipwell-specific output configuration: how resolution, bitrate, codecs, pixel formats, and color profiles are implemented. For general background on these concepts, see [concepts/VIDEO.md](../concepts/VIDEO.md) and [concepts/AUDIO.md](../concepts/AUDIO.md).

## Resolution

Clipwell supports two capture resolutions, controlled by the **Capture Native Resolution** setting (enabled by default):
...
Clipwell automatically adjusts settings when changing formats to maintain compatibility:
...
Clipwell uses the `HDRPreset` enum to select the correct ScreenCaptureKit configuration:
...
The macOS 26+ preset injects static HDR10 metadata and configures all color properties as a validated unit. On older macOS versions, Clipwell manually configures `captureDynamicRange` and the codec-appropriate pixel format.
