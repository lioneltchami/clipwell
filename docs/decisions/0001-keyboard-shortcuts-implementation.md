...
Clipwell has no keyboard shortcuts.
All actions (toggle recording, select content, select area) require clicking through the menu bar popover.
Users have requested global hotkeys that work even when the app is not focused (see [Discussion #76](https://github.com/lioneltchami/clipwell/discussions/76), [Issue #119](https://github.com/lioneltchami/clipwell/issues/119)).

## Decision Drivers

- Clipwell is fully sandboxed (`com.apple.security.app-sandbox`) and must remain so for Mac App Store compatibility.
