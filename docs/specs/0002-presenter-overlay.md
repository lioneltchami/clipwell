...
Screen recordings often lack a personal touch. macOS provides Presenter Overlay, a system-level feature that composites the presenter's camera feed on top of shared content (small movable window or large immersive mode). Clipwell currently has no camera integration, so Presenter Overlay is unavailable. Adding camera support enables this feature automatically through the system's Video menu bar item.

## Expected outcome

- A new "Presenter Overlay" toggle appears in the menu bar settings under a "Camera" section.
- When enabled and a recording starts, Clipwell starts an `AVCaptureSession` for the selected camera.
...
- The user controls overlay mode (small/large/off) through the system Video menu bar — Clipwell does not need its own overlay controls.
...
1. **Entitlement**: `com.apple.security.device.camera` in `Clipwell.entitlements`.
...
| File | Change |
|------|--------|
| `Clipwell.entitlements` | Add `com.apple.security.device.camera` entitlement |
