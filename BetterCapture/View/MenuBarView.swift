        ...
        .sheet(isPresented: Binding(
            ...
            if let url = viewModel.lastRecordingURL {
                PostRecordingSheet(
                    ...
                    onDismiss: { viewModel.dismissPostRecordingSheet() }
                )
            }
        }
    }
    ...
    var body: some View {
        VStack(spacing: 4) {
            ...
            if permissionService.screenRecordingState != .granted {
                ...
                Text("Enable **CaptureForge** in the list — not the old BetterCapture entries.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .padding(.horizontal, 12)
                    .padding(.bottom, 4)
            ...
            if showMicrophonePermission && permissionService.microphoneState != .granted {
                ...
                Text("Click **Allow Microphone** first. CaptureForge appears in Settings only after you approve the system dialog.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .padding(.horizontal, 12)
                    .padding(.bottom, 4)
