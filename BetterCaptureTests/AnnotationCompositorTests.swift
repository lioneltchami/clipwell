...
struct AnnotationCompositorTests {
    ...
    @Test func compositingStrokeChangesPixelBuffer() throws {
        ...
        let before = samplePixel(buffer, x: width / 2, y: height / 2)
        ...
        let after = samplePixel(buffer, x: width / 2, y: height / 2)
        #expect(before != after)
    ...
    @Test func emptyStrokeListIsNoOp() throws {
        ...
        let before = samplePixel(buffer, x: 8, y: 8)

        AnnotationCompositor.composite(strokes: [], onto: buffer)

        #expect(samplePixel(buffer, x: 8, y: 8) == before)
    }
    ...
    private func samplePixel(_ buffer: CVPixelBuffer, x: Int, y: Int) -> Pixel {
