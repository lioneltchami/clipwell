...
struct AssetWriterTests {
    ...
    /// Pause compensation shifts the session anchor so resumed frames stay contiguous.
    @Test func pauseCompensationKeepsTimelineContiguous() async throws {
        let settings = makeStore()
        settings.frameRate = .fps30
        ...
        // 0.0s, 0.033s, then a 1s wall-clock pause, then frames continue at 1.067s, 1.1s
        assetWriter.appendVideoSample(try makeVideoSampleBuffer(at: CMTime(value: 0, timescale: 30)))
        assetWriter.appendVideoSample(try makeVideoSampleBuffer(at: CMTime(value: 1, timescale: 30)))
        assetWriter.compensateForPause(duration: 1.0)
        assetWriter.appendVideoSample(try makeVideoSampleBuffer(at: CMTime(value: 32, timescale: 30)))
        assetWriter.appendVideoSample(try makeVideoSampleBuffer(at: CMTime(value: 33, timescale: 30)))
        ...
        // Without compensation, wall gap would leave ~1s hole. Contiguous CFR
        // output should stay near the written frame span (~0.1s), not ~1s+.
        let span = CMTimeSubtract(times.last!, times.first!).seconds
        #expect(span < 0.5)
    }

    @Test func annotationStateBurnsIntoWrittenFrames() async throws {
        let settings = makeStore()
        settings.frameRate = .fps30

        let annotationState = AnnotationState()
        annotationState.addStroke(
            AnnotationStroke(points: [
                AnnotationPoint(x: 0.0, y: 0.5),
                AnnotationPoint(x: 1.0, y: 0.5)
            ])
        )

        let assetWriter = AssetWriter()
        assetWriter.annotationState = annotationState
        try assetWriter.setup(url: makeOutputURL(), settings: settings, videoSize: videoSize)
        try assetWriter.startWriting()
    ...
    private func makeStore() -> SettingsStore {
        let suiteName = "com.lionel.CaptureForgeTests.\(UUID().uuidString)"
